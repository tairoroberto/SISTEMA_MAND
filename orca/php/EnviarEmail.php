<?php
   ...
   function enviarEmail($email,$msg,$IdOportunidade,$IdOrcamentoB){

	$buscaOportunidade = new Conexao();
    $buscaOportunidade->conectar();
    $buscaOportunidade->selecionarDB(); 

    $buscaLinkOrcamento = new Conexao();
    $buscaLinkOrcamento->conectar();
    $buscaLinkOrcamento->selecionarDB(); 
    ...
    $buscaLinkOrcamento->set('sql',"SELECT * FROM LinkOrcamentoB
                                    WHERE IdOportunidade = $IdOportunidade AND 
                                          IdOrcamentoB = $IdOrcamentoB ");

    $retornoLinkOrcamento = mysql_fetch_object($buscaLinkOrcamento->executarQuery()); 
    ...
    // =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
    $mail->Subject  = "Mand | Resposta de orçamento enviado"; // Assunto da mensagem
        ...
        <table class='table_scale' width='' border='0' align='center' cellpadding='0' cellspacing='0' bgcolor='#ffffff' style='padding:0; margin: 0;'>
          <tr>
            <td class='full_width' align='left' valign='top' width='600' bgcolor='#ffffff' style=''><table width='100%'>
                ...
                <!-- START OF HEADING-->
                <tr>
                  <td align='left' valign='top' style='padding-top: 23px; padding-right: 30px; padding-bottom: 0; padding-left: 30px; font-size:16px ; line-height: 34px; text-transform: none; color:#1f2122; font-family: Arial,sans-serif; '> Retorno Orçamento ".$retornoOportunidade->RazaoSocial."</td>
                </tr>
                <!-- END OF HEADING-->
                <!-- START OF TEXT-->
                <tr>
                  <td align='left' valign='top' style='padding-top: 4px; padding-right: 30px; padding-bottom: 0; padding-left: 30px; font-size:14px ; color:#999b9e; font-family: Arial,sans-serif; line-height: 24px; text-align:justify; '> 
                    ...
                    <br />
                    <br />
                    O cliente ".$retornoOportunidade->RazaoSocial." ".$msg.".
                    <br />
                    <br />
                  <strong>Link: <a href='http://www.mandprojetos.com.br/".$retornoLinkOrcamento->Endereco."' target = '_blank'> Orçamento  </a><br />
                  <br />
                  </strong></td>
  ...
  <!-- END FOOTER AREA-->
    ...
    $mail->ClearAttachments();
      ...
      if ($enviado) { 
        ...
        /********************************************************************************************/
         $atualizarOportunidade->set('sql',"UPDATE Oportunidade SET Status = 'Orçamento Aceito'
       ...
       $insereHistoricoOportunidade->set('sql',"INSERT INTO HistoricoOportunidade(DataHistoricoOportunidade, TipoHistoricoOportunidade, Status, IdOportunidade) 
                                                 VALUES ('$data','Alterado','Orçamento aceito pelo cliente','$IdOportunidade');");
       $insereHistoricoOportunidade->executarQuery();
        ...
        /********************************************************************************************/
         $atualizarOportunidade->set('sql',"UPDATE Oportunidade SET Status = 'Erro: cliente não consegue aceitar orçamento'
                                                                WHERE IdOportunidade = '$IdOportunidade';");
         $atualizarOportunidade->executarQuery();
         ...
         $data = date('d/m/Y');
       $insereHistoricoOportunidade->set('sql',"INSERT INTO HistoricoOportunidade(DataHistoricoOportunidade, TipoHistoricoOportunidade, Status, IdOportunidade) 
                                                 VALUES ('$data','Alterado','Cliente encontrou problemas ao aceitar orçamento','$IdOportunidade');");
       $insereHistoricoOportunidade->executarQuery();
